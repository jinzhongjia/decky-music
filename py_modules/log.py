...
import decky
from diagnostics import safe_category, safe_code
...
def log_child_event(source, event):
    # Category and stable codes retain context without trusting child-controlled prose.
    category = safe_category(event.where)
    code = safe_code(event.msg, "")
    message = code or "child diagnostic"
    log(source, "socket", event.level, f"{category}: {message}")
...
#
# Known probe noise keeps its debug severity; only a fixed summary is retained.
# Unknown stderr stays warn, but neither path records raw native/library text.
_STDERR_NOISE = (
...
async def pump_stderr(source: str, stream):
    """Summarize stderr; arbitrary native/library output may contain credentials."""
    while stream and (line := await stream.readline()):
        text = line.decode(errors="replace").rstrip()
        if text:
            level = stderr_level(text)
            summary = "audio probe diagnostic" if level == "debug" else "unexpected child stderr"
            log(source, "stderr", level, summary)
...
def dir_size(path: str) -> int:
    ...
    try:
        return sum(
            os.path.getsize(e.path) for e in os.scandir(path) if e.is_file(follow_symlinks=False)
        )
