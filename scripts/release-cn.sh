...
put() {  # $1=local file  $2=r2 key (no bucket prefix)
  if [ "${DRY_RUN:-}" = "1" ]; then echo "DRY put -> $2"; return; fi
  npx --yes wrangler r2 object put "$R2_BUCKET/$2" --file="$1" --remote
}
...
put "$work/Decky.Music.cn.zip" "decky_music/decky-music-cn.zip"
