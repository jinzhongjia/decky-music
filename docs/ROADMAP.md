...
## NCM 红心业务码校验（v1.0.8 后，已设备验证）

- 仅在 `like_song` 响应边界检查原始业务码；数字/字符串 `200` 才成功，
  被 SDK 包装为成功的业务失败及缺失业务码均返回安全的 `provider_error`，原有超时错误保持不变。
- 共享请求层、扫码状态及歌单写操作的独立语义未改动；前端/bridge 的参数、响应字段和错误码契约不变。
- 业务码 `502` 被包装成成功、缺少业务码两项回归在修复前失败；NCM 37 项测试、Clippy 与 ABI 检查通过。
- 用重建二进制在设备隔离验证：正常收藏数 6→7→6，原收藏集合完全恢复；
  同一无效歌曲 ID `0` 从此前的 `ok:true` 变为 `ok:false / provider_error`。
- 已授权更新设备 NCM 二进制，SHA-256 为 `1f4d852cbc712cf928a46b8829e1742be1efed01dcf213746d8f058db01bdff5`；
  当前 QQ 音源未切换，未重启 QQ、Steam 或插件。既有 v1.0.8 安装包尚不包含此修复。

---

## QQ 红心收藏修复（v1.0.8，已真机验证）

- `query_song` 改传 SDK 的 `SongQueryInfo`，不再把 MID 字符串当查询对象；红心与添加到歌单共用的查询路径一并修复。
- 收藏键固定使用所查询的 QQ 曲库 `songType=0`，不再误传详情中表示普通歌曲的 `type=1`；
  后者会让取消操作返回成功但实际保留歌曲。真实歌单 TID、保留布尔值的探针未解决问题，不保留这些无效改动。
- 三项回归通过真实 SDK 查询/写入方法验证 MID 解析、收藏往返和已有 QQ 曲库条目的删除；
  字符串参数与错误类型分别在修复前复现。QQ 完整回归 92 项、Ruff 与 ABI 检查通过。
- 已重建并经授权仅替换 QQ provider，保留 bridge/player、电台队列、当前曲约 7.09 秒暂停位置及 55% 音量。
  实机点击取消/添加后分别从 QQ 收藏列表读回不存在/存在，最终保留红心；独立真实请求的收藏数为 317→316→317。
- 真机验收所用 QQ 可执行文件 SHA-256：`7a964f239a728f8d016dc21abd2c0de4797ce6f3680dedff8ff85df222afc14d`。
  本轮修复归入 v1.0.8；既有 v1.0.7 下载包不含修复，不能覆盖后仍宣称已更新。
- v1.0.8 发布 QQ 可执行文件 SHA-256：`e982ffb11036fbec3e75d049ef616b41a2a14218179e3390e5c0dc5443cb9ccd`；
  已在设备隔离进程复验取消/恢复收藏，并确认原收藏成员及顺序一致，未替换正在使用的插件。
- 调试中另遇 Steam 休眠遮罩占据焦点：事件循环仍响应，系统未处于 suspend，但 Steam 保持 suspending。
  解除防休眠并调用 Steam 自身恢复流程后，遮罩消失、左右焦点导航恢复；未重启 Steam 或退出账号。
- 同步移除侧载脚本对根目录 `dist/` 产物的旧权限处理假设；打包入口实际只交付 `out/`。
...
- 最终版本已由用户确认“声音正常”；已恢复原 QQ 曲目约 111 秒、55% 插件音量并暂停。
  临时实例与设备脚本已删除，防休眠已解除；现有账号没有被退出。
