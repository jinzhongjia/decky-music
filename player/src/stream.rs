...
impl BufferState {
    ...
    fn fail(&mut self, reason: &'static str) {
        ...
        // Invalidate an HTTP open/read still running outside the lock. Keep the producer
        // alive so an out-of-buffer seek can reset the failure and restart fetching.
        self.generation += 1;
    }
...
impl HttpRangeReader {
    fn open_url(url: &str) -> Result<Self, Box<dyn std::error::Error + Send + Sync>> {
        ...
        thread::spawn({
            ...
            move || producer_loop(shared, client, url, Some(response), 0, Read::read)
        });
        Ok(Self { shared })
    }
...
impl HttpRangeReader {
    fn read_with_wait(
        ...
        loop {
            ...
            // Recheck data/error/EOF after reacquiring the lock, even if the wait
            // expired: the producer may have made progress at the deadline.
            if timed_out {
...
fn producer_loop(
    ...
    mut read_response: impl FnMut(&mut Response, &mut [u8]) -> io::Result<usize>,
) {
    ...
    loop {
        ...
        let read = response
            .as_mut()
            .map(|resp| read_response(resp, &mut chunk));
        let mut state = shared.state.lock();
        if state.generation != response_generation {
            response = None;
            continue;
