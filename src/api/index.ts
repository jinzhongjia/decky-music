...
export const getSongUrl = callable<
  [mid: string, preferredQuality?: PreferredQuality],
  SongUrlResponse
>("get_song_url");

/** 获取歌词 (qrc=true 时获取 QRC 逐字格式) */
export const getSongLyric = callable<[mid: string, qrc?: boolean], SongLyricResponse>(
  "get_song_lyric"
);
...
/** 获取所有可用的 Provider */
export const getProviders = callable<[], ProvidersResponse>("get_providers");

/** 获取当前激活的 Provider */
export const getCurrentProvider = callable<[], CurrentProviderResponse>("get_current_provider");

/** 切换 Provider */
export const switchProvider = callable<[provider_id: string], SwitchProviderResponse>(
  "switch_provider"
);
